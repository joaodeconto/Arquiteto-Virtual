...
public class I18n {
	...
	private void InitializeEnglish()
	{
		...
		hashWords [1] ["Profundidade"]                 	= "Width";
	...
	private bool wasUpdatedLastLanguage = false;

	public void ChangeLanguage(int selectedLanguageIndex)
	{
		if(this.selectedLanguage == selectedLanguageIndex)
		{
			Debug.Log("não entrou...");
			Debug.Log("estava: " + this.selectedLanguage + " deveria trocar para: " + selectedLanguageIndex);
			return;
		}

		Debug.Log("estava: " + this.selectedLanguage + " trocando para: " + selectedLanguageIndex);

		lastLanguage          = this.selectedLanguage;
		this.selectedLanguage = selectedLanguageIndex;

		if (wasUpdatedLastLanguage)
		{
			wasUpdatedLastLanguage = true;
			return;
		}

		//for each language
			//add new indices
			//save list of old indices
			//remove old indices
		...
		for(int i = 0; i != hashWords.Length; ++i)
		{
			if(i != lastLanguage)
			{
				foreach(DictionaryEntry deNew in hashWords[this.lastLanguage])
				{
					hashWords[i][(string)deNew.Value] = (string)hashWords[i][(string)deNew.Key];
					hashWords[i].Remove(deNew.Key);

					//Debug.Log("chegou velha key: " + (string)deNew.Key + " nova key: " + (string)deNew.Value);
				}
			}
		}
		...
		foreach(DictionaryEntry deNew in hashWords[this.lastLanguage])
		{
			newHash[(string)deNew.Value] = (string)deNew.Value;
		}

		hashWords[lastLanguage].Clear();
		hashWords[lastLanguage] = newHash;
	}
	...
	static private I18n instance = null;
