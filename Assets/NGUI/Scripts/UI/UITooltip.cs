	...
	void SetText (string tooltipText)
	{
		if (text != null && !string.IsNullOrEmpty(tooltipText))
		{
			...
			if (background != null)
			{
				...
				mSize.x *= textScale.x;
				mSize.y *= textScale.y;
				mSize.x += (background.border.x + background.border.z + ( offset.x - background.border.x) * 2f) + offsetSize.x;
				mSize.y += (background.border.y + background.border.w + (-offset.y - background.border.y) * 2f) + offsetSize.y;
				mSize.z = 1f;
